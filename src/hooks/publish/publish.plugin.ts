...
import type { ChannelConfiguration, PluginConfiguration } from "../../configuration";
import { deploy } from "./deploy";
import { fulfillClientOptions } from "./fullfil-client-options";
...
export async function publish(
  ...
  const { channels, globalOptions: options } = fulfillOptions(
    pluginConfiguration
  );

  const channelConfiguration: ChannelConfiguration | undefined = getChannelConfiguration(
    nextRelease,
    channels ?? []
  );
  ...
  const clientOptions: VercelClientOptions = fulfillClientOptions(context, options);
  const deploymentOptions: DeploymentOptions = fulfillDeploymentOptions(context, channelConfiguration, options);
