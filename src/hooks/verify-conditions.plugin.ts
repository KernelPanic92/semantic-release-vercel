...
export async function verifyConditions(
  ...
  const vercelToken = globalOptions?.client?.token ?? context.env[VERCEL_TOKEN_ENV_NAME];
  ...
  const vercelTeamId = globalOptions?.client?.teamId ?? context.env[VERCEL_TEAM_ID_ENV_NAME];
